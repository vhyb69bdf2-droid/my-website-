...
initSegmented("multiJoinSeg", v => {
  ...
  ["multiDigits","multiRepeatWrap","multiModeSeg"].forEach(id => {
...
initSegmented("groupJoinSeg", v => {
  ...
  ["groupSizeWrap","groupDigitsWrap","groupRepeatWrap","groupModeWrap","groupVisWrap"].forEach(id => {
...
const Nova = (() => {
  ...
  return {
    init(len, diff, allowRepeats = false) {
      difficulty = diff;
...
const NovaGame = (() => {
  ...
  function start(name, digits, diff, mode, allowRepeats) {
    reset();
    ...
    Nova.init(state.digits, diff, state.allowRepeats);
  ...
  function nextRound() {
    ...
    state.novaSecret = randomCode(state.digits, state.allowRepeats);
    state.timerSecs = 180;
    Nova.init(state.digits, state.difficulty, state.allowRepeats);
...
document.getElementById("novaStartBtn").addEventListener("click", e => {
  ...
  NovaGame.start("You", digits, diff, mode, allowRepeats);
});
...
const MultiGame = (() => {
  ...
  function createRoom(name, digits, mode, allowRepeats) {
    reset();
...
document.getElementById("multiStartBtn").addEventListener("click", e => {
  ...
  withDatabase("Multiplayer", () => {
    if (joinType === "create") {
      MultiGame.createRoom("Player 1", digits, mode, allowRepeats);
    } else {
...
const GroupGame = (() => {
  ...
  function createRoom(name, digits, playerCount, mode, visibility, allowRepeats) {
    reset();
    ...
    const roomData = {
      digits: state.digits, allowRepeats: state.allowRepeats, playerCount: state.playerCount, mode: state.mode,
  ...
  function nextGroupRound() {
    ...
    setTimeout(() => startGroupGame({
      ...
      digits: state.digits, allowRepeats: state.allowRepeats, visibility: state.visibility,
...
document.getElementById("groupStartBtn").addEventListener("click", e => {
  ...
  withDatabase("Group Race", () => {
    if (joinType === "create") {
      ...
      GroupGame.createRoom(name, digits, playerCount, mode, vis, allowRepeats);
    } else {
