...
const GroupGame = (() => {
  ...
  function buildTargetSelect() {
    ...
    const targetIds = lockedTarget
      ? [lockedTarget]
      : Object.keys(state.players).filter(pid => {
  ...
  function submitGuess(raw) {
    ...
    // Advance turn
    if (state.mode === "turns") {
      const pids = activeOvertimePlayers() || Object.keys(state.players).sort();
  ...
  function changeGroupSettings() {
    ...
    if (createBtn) createBtn.click();
    document.getElementById("groupSize").value = String(Object.keys(state.players || {}).length || state.playerCount);
  ...
  function applySettingsNextRound() {
    state.playerCount = Object.keys(state.players || {}).length || state.playerCount;
  ...
  function quit() {
    ...
    if (isBattleOvertime && db && state.roomCode) {
      ...
      fbRef(`groups/${state.roomCode}/phase`).set(`abandoned:${state.playerId || "unknown"}`);
    }
    showPage("pageHome");
  }
