...
const GroupGame = (() => {
  ...
  function createRoom(name, digits, playerCount, mode, visibility, allowRepeats, clueMode, groupType) {
    ...
    const myEntry = { name: state.playerName, ready: true, locked: false, secret: "", connected: true };
    const roomData = {
  ...
  function joinRoom(name, code) {
    ...
    fbRef(`groups/${state.roomCode}`).once("value", snap => {
      ...
      fbRef(`groups/${state.roomCode}/players/${state.playerId}`).set({ name: state.playerName, ready: true, locked: false, secret: "", connected: true });
