...
const NovaGame = (() => {
  ...
  function start(name, digits, diff, mode) {
    reset();
    ...
    connectDigitPad("novaDigitGrid", "novaGuessInput", () => false);
    renderNovaUI();
...
document.getElementById("novaStartBtn").addEventListener("click", e => {
  ...
  NovaGame.start("You", digits, diff, mode);
});
...
const GroupGame = (() => {
  ...
  function nextGroupRound() {
    ...
    fbRef(`groups/${state.roomCode}`).update({
      ...
      secret: newSecret, allowRepeats: state.allowRepeats, currentTurn: pids[0], guesses: {}
    });
