...
function renderHistoryItemPublic(guess, greens, yellows, guessLabel) {
  ...
  li.innerHTML = `<div class="guess-number">${guessLabel}</div>
    <div style="display:grid;gap:6px">
      ...
      <div style="font-size:.78rem;color:var(--muted)">${greens} 🟩 ${yellows} 🟨</div>
    </div>`;
  return li;
}
...
document.getElementById("novaLockBtn").addEventListener("click", () => NovaGame.lockSecret());
document.getElementById("novaGuessBtn").addEventListener("click", () => {
...
document.getElementById("novaGuessInput").addEventListener("keydown", e => {
  if (e.key === "Enter") NovaGame.submitGuess(document.getElementById("novaGuessInput").value);
});

document.getElementById("novaStartBtn").addEventListener("click", () => {
...
const MultiGame = (() => {
  ...
  function joinRoom(name, code) {
    ...
    roomRef.once("value", snap => {
      ...
      roomRef.child("p2").update({ name: state.playerName });
      roomRef.child("phase").set("lobby");

      const off = roomRef.on("value", snap2 => {
        const d2 = snap2.val();
        if (!d2) return;
        if (d2.phase === "lock") {
          off(); clearListeners();
  ...
  function startGamePhase(data) {
    ...
    // Reset Firebase guesses for new round
    const roomRef = fbRef(`rooms/${state.roomCode}`);
    if (state.playerId === "p1") {
      roomRef.child("guesses").set({ p1: {}, p2: {} });
      roomRef.child("p1").update({ locked: false, secret: "" });
      roomRef.child("p2").update({ locked: false });
    }

    setupMultiGameUI();
    listenForOpponentLock();
  }
  ...
  function listenForOpponentLock() {
    const roomRef = fbRef(`rooms/${state.roomCode}`);
    const oppId = state.playerId === "p1" ? "p2" : "p1";
    const off = roomRef.child(oppId).on("value", snap => {
      const d = snap.val();
      if (d && d.locked) {
    ...
    listeners.push(() => roomRef.child(oppId).off("value", off));
  }

  function checkBothLocked() {
    if (state.myLocked && state.oppLocked) {
      // Both locked — start the game
      const roomRef = fbRef(`rooms/${state.roomCode}`);
      ...
      // Fetch opponent secret if p1 (p1 knows p2's secret to score, vice versa)
      fetchOppSecret();
      startTurnListener();
  ...
  function startTurnListener() {
    ...
    const off3 = roomRef.child("phase").on("value", snap => {
      const phase = snap.val();
      if (phase && phase.startsWith("win:")) {
        const winner = phase.split(":")[1];
        handleWin(winner);
      } else if (phase === "draw") {
        handleDraw();
      }
    });
    listeners.push(() => roomRef.child("phase").off("value", off3));
  }
  ...
  function handleWin(winnerId) {
    ...
    showRoundEnd(
      ...
      `The code was ${state.oppSecret || "???"}. Score: ${state.p1Score} – ${state.p2Score}`,
      [
        { label: "Next Round", primary: true, action: () => nextRound() },
  ...
  function startRealtimeTimer() {
    ...
    fbRef(`rooms/${state.roomCode}`).child("timerEnd").set(end);
  ...
  function nextRound() {
    ...
    stopTimer();
    const roomRef = fbRef(`rooms/${state.roomCode}`);
    roomRef.update({ round: state.round, phase: "lock", currentTurn: "p1" });
...
document.getElementById("multiStartBtn").addEventListener("click", () => {
...
document.getElementById("multiLockBtn").addEventListener("click", () => MultiGame.lockSecret());
document.getElementById("multiGuessBtn").addEventListener("click", () => {
...
document.getElementById("multiGuessInput").addEventListener("keydown", e => {
  if (e.key === "Enter") MultiGame.submitGuess(document.getElementById("multiGuessInput").value);
});
...
document.getElementById("groupGuessBtn").addEventListener("click", () => {
...
document.getElementById("groupGuessInput").addEventListener("keydown", e => {
  if (e.key === "Enter") GroupGame.submitGuess(document.getElementById("groupGuessInput").value);
});
