      ...
      s3.onload = () => {
        ...
        if (FB_CFG.apiKey && FB_CFG.projectId) {
          ...
          auth.signInAnonymously().catch(err => {
            console.warn("Anonymous auth failed:", err && err.message ? err.message : err);
          });
        }
      };
...
function withDatabase(label, cb) {
  ...
  const wait = setInterval(() => {
    ...
    if (db && (!auth || authUid)) {
      ...
      clearInterval(wait);
      alert(`${label} is still connecting to Firebase${auth ? " Auth" : ""}. Try again in a moment.`);
    }
  }, 100);
}
...
const MultiGame = (() => {
  ...
  function joinRoom(name, code) {
    ...
    roomRef.once("value", snap => {
      ...
      roomRef.child("p2").update({ name: state.playerName, locked: false, secret: "", uid: authUid || "" });
...
const GroupGame = (() => {
  ...
  function joinRoom(name, code) {
    ...
    fbRef(`groups/${state.roomCode}`).once("value", snap => {
      ...
      fbRef(`groups/${state.roomCode}/players/${state.playerId}`).set({ name: state.playerName, ready: true, locked: false, secret: "", connected: true, uid: authUid || "" });
