...
function renderHistoryItemPublic(guess, greens, yellows, guessLabel, clueMode = "position") {
  ...
  li.innerHTML = `<div class="guess-number">${guessLabel}</div>
    <div style="display:grid;gap:6px">
      ...
      <div style="font-size:.78rem;color:var(--muted)">${clueMode === "present" ? `${yellows} present` : `${greens} green / ${yellows} yellow`}</div>
    </div>`;
  return li;
}
...
initSegmented("groupJoinSeg", v => {
  ...
  ["groupSizeWrap","groupDigitsWrap","groupRepeatWrap","groupModeWrap","groupVisWrap","groupClueWrap"].forEach(id => {
...
const GroupGame = (() => {
  ...
  function createRoom(name, digits, playerCount, mode, visibility, allowRepeats, clueMode) {
    reset();
    ...
    const roomData = {
      digits: state.digits, allowRepeats: state.allowRepeats, clueMode: state.clueMode, playerCount: state.playerCount, mode: state.mode,
  ...
  function submitGuess(raw) {
    ...
    const { greens, yellows } = scoreGuess(state.secret, guess, state.clueMode);
    const entry = { guess, greens, yellows, ts: Date.now() };
  ...
  function renderGroupGuesses() {
    Object.entries(state.guesses).forEach(([pid, gdata]) => {
      ...
      arr.forEach((g, i) => {
        if (isMe) {
          list.appendChild(renderHistoryItem(state.secret, g.guess, `#${i+1} — ${g.guess}`, state.clueMode));
        } else {
          list.appendChild(renderHistoryItemPublic(g.guess, g.greens, g.yellows, `#${i+1} — ${g.guess}`, state.clueMode));
        }
      });
    });
  }
  ...
  function nextGroupRound() {
    ...
    setTimeout(() => startGroupGame({
      ...
      digits: state.digits, allowRepeats: state.allowRepeats, clueMode: state.clueMode, visibility: state.visibility,
...
document.getElementById("groupStartBtn").addEventListener("click", e => {
  ...
  withDatabase("Group Race", () => {
    if (joinType === "create") {
      ...
      GroupGame.createRoom(name, digits, playerCount, mode, vis, allowRepeats, clueMode);
    } else {
