...
const MultiGame = (() => {
  ...
  function createRoom(name, digits, mode, allowRepeats, clueMode, trackerEnabled) {
    ...
    roomRef.child("p1/connected").set(true);
  ...
  function quit() {
    ...
    if (state.roomCode && db && !state.gameOver) {
  ...
  return { createRoom, joinRoom, lockSecret, submitGuess, quit };
})();
...
const GroupGame = (() => {
  ...
  function createRoom(name, digits, playerCount, mode, visibility, allowRepeats, clueMode, groupType) {
    ...
    const myEntry = { name: state.playerName, ready: true, locked: false, secret: "" };
    const roomData = {
  ...
  function joinRoom(name, code) {
    ...
    fbRef(`groups/${state.roomCode}`).once("value", snap => {
      ...
      fbRef(`groups/${state.roomCode}/players/${state.playerId}`).set({ name: state.playerName, ready: true, locked: false, secret: "" });
  ...
  function setupGroupGameUI() {
    ...
    document.getElementById("groupRulesBar").textContent = settingsSummary(state);
    ...
    document.getElementById("groupSecretPanel").style.display = state.groupType === "battle" && !state.myLocked ? "grid" : "none";
    document.getElementById("groupSecretLocked").style.display = state.groupType === "battle" && state.myLocked ? "block" : "none";
    document.getElementById("groupSecretLocked").textContent = state.groupType === "battle" && state.myLocked ? `Your secret: ${state.mySecret || "locked"}` : "Your secret is locked. Waiting for the rest of the room...";
    document.getElementById("groupSecretHelp").textContent = `Choose your secret code. ${digitRuleText(state.allowRepeats)}`;
    document.getElementById("groupSecretInput").maxLength = state.digits;
    document.getElementById("groupSecretInput").value = "";
    document.getElementById("groupSecretInput").disabled = false;
    document.getElementById("groupLockBtn").disabled = false;
    document.getElementById("groupSecretInput").placeholder = `e.g. ${randomCode(state.digits, state.allowRepeats)}`;
    document.getElementById("groupTargetWrap").style.display = state.groupType === "battle" ? "block" : "none";
    buildTargetSelect();
    buildGroupHistories();
    updateGroupTurnUI();
  ...
  function listenForGroupRoom() {
    ...
    const off = roomRef.on("value", snap => {
      ...
      if (state.groupType === "battle" && data.phase === "lock") {
        const allLocked = Object.values(state.players).length >= state.playerCount && Object.values(state.players).every(p => p.locked);
        ...
        document.getElementById("groupSecretPanel").style.display = state.myLocked ? "none" : "grid";
        document.getElementById("groupSecretLocked").style.display = state.myLocked ? "block" : "none";
      ...
      if (state.groupType === "battle" && data.phase === "playing") {
        document.getElementById("groupStatusBar").textContent = "Pick a target, make a guess, and score 1 point for each code you crack.";
        document.getElementById("groupSecretPanel").style.display = "none";
        document.getElementById("groupSecretLocked").style.display = "block";
  ...
  function updateGroupTurnUI() {
    ...
    if (state.mode === "turns") {
      ...
      label.textContent = state.groupType === "battle" ? "Real-Time - choose any target." : "Real-Time — guess whenever!";
    }
  }
  ...
  function submitGuess(raw) {
    ...
    const targetId = state.groupType === "battle" ? document.getElementById("groupTargetSelect").value : null;
    ...
    const targetSecret = state.groupType === "battle" ? (state.players[targetId] && state.players[targetId].secret) : state.secret;
    if (!targetSecret) {
    ...
    if (state.groupType === "battle") {
      const ok = window.confirm(`Target ${targetName} with ${guess}?`);
      if (!ok) return;
    }
    ...
    if (greens === state.digits) {
      ...
      if (state.groupType === "battle") {
        ...
        const crackedNow = { ...state.cracked, [`${state.playerId}_${targetId}`]: true };
        const possibleCracks = pids.length * Math.max(0, pids.length - 1);
        if (Object.keys(crackedNow).length >= possibleCracks) {
          const scoresNow = { ...state.scores, [state.playerId]: newScore };
          const winner = Object.entries(scoresNow).sort((a, b) => (b[1] || 0) - (a[1] || 0))[0];
          if (winner) fbRef(`groups/${state.roomCode}/phase`).set(`win:${winner[0]}`);
          return;
        }
      } else {
  ...
  function startGroupOvertime() {
    ...
    document.getElementById("roundEndMsg").textContent = "Overtime vote sent. Waiting for the rest of the room...";
  }
  ...
  return { createRoom, joinRoom, submitGuess, lockSecret };
})();
...
document.getElementById("groupStartBtn").addEventListener("click", e => {
  ...
  withDatabase("Group Race", () => {
    if (joinType === "create") {
      ...
      GroupGame.createRoom(name, digits, playerCount, mode, vis, allowRepeats, clueMode, groupType);
    } else {
