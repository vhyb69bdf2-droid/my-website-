...
const GroupGame = (() => {
  ...
  function renderGroupScoreboard() {
    ...
    el.innerHTML = Object.entries(state.players).map(([pid, p]) => {
  ...
  function listenForTurn() {
    ...
    const handleRoomValue = snap => {
      ...
      if (data.phase === "draw" && data.overtimeVotes) {
        const players = data.overtimePlayers || Object.keys(data.players || {});
  ...
  function isBattleOvertimeWatcher() {
    return state.groupType === "battle" && Array.isArray(state.overtimePlayers) && !state.overtimePlayers.includes(state.playerId);
  }

  function battleWinTarget() {
    return Math.max(1, Object.keys(state.players || {}).length - 1);
  }
  ...
  function handleGroupDraw() {
    ...
    if (isBattleOvertimeWatcher()) {
      ...
      showRoundEnd("Overtime Tiebreaker", `${activeNames} are tied for first. You are watching this overtime.`,
        [
          { label: "Watch", primary: true, action: () => { updateGroupTurnUI(); } },
          { label: "Quit", primary: false, action: () => groupQuit() }
        ]
      );
      return;
  ...
  function startGroupOvertime() {
    ...
    document.getElementById("roundEndMsg").textContent = state.overtimePlayers
      ? "Overtime vote sent. Waiting for the other tied player..."
